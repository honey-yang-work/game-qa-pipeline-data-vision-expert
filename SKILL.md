...
- Phase 0-4 为标准流水线；Phase 5 为 Phase 0-4 或 Phase 4R 最新基线确认后的后端工程 + 配置表联合白盒闭环，默认源码根为 `C:\NuoYaIdle\Server\`，默认配置根为 `C:\NuoYaIdle\Tools\表格配置及转换\excel\`，用户提供的源码片段、路径、文件夹或压缩包仅作为补充输入。
...
- 若用户同时提供新版策划文档与源码更新，必须先完成 Phase 4R 锁定最新需求基线，再进入 Phase 5；不得跳过 Phase 4R 直接对白盒范围下结论。
...
在 Phase 0-4 或 Phase 4R 完成需求理解、RTM 建模和用例设计后，若需要白盒闭环审计，默认进入 `C:\NuoYaIdle\Server\` 后端工程和 `C:\NuoYaIdle\Tools\表格配置及转换\excel\` 配置表目录，联合定位本期需求承载代码、配置字段和值；用户提供的源码片段、本地路径、文件夹或压缩包作为补充输入。若存在 Phase 4R 最新确认资产，Phase 5 必须以最新基线为准，不得回退到旧版策划文档对应的 RTM、用例范围或结构化载荷。Step 0-4 必须逐步确认。详细规则见 `references/whitebox-audit-phase5.md`。
